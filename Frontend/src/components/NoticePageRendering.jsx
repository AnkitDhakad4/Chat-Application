...
function MessageRequest({rqst}){

    const {acceptMessageReqeust,rejectMessageReqeust}=requestStore();

    const accepted=async()=>{
        console.log("In accepted")
        await acceptMessageReqeust(rqst)
    }

    const rejected=async()=>{
        console.log("In rejected")
        await rejectMessageReqeust(rqst)
    }
    return (
        <div className="rung h-1/10 flex p-0.5 items-center justify-evenly ">
...
function NoticePageRendering() {
  ...
  const rqst=<><h1>This is H1 tag sended as children</h1></>
  return (
