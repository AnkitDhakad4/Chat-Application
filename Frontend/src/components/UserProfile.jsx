...
function UserProfile( {setprofileViewer} ) {
  ...
  const getInitialDob = (incomingDob) => {
    if (!incomingDob) return "";

    const parsedDate = new Date(incomingDob);
    
    
    if (isNaN(parsedDate.getTime())) {
      console.warn("Warning: Received invalid date format from database:", incomingDob);
      return ""; 
    }
  ...
  const handleChange = (e) => {
    ...
    setProfile(prev => ({ ...prev, [name]: name === "dob" ? toDDMMYYYY(value) : value }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    let finalProfilePayload = { ...profile };
    ...
    finalProfilePayload.dob = toYYYYMMDD(finalProfilePayload.dob);
      ...
      <div className="relative w-full max-w-4xl bg-[#F0F0F0] rounded-2xl shadow-2xl overflow-hidden text-slate-800 flex flex-col md:flex-row border border-white/20">
        ...
        <div className="w-full md:w-[45%] bg-[#E5E5E5] p-8 flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-slate-300/50">
          ...
          <button 
            ...
            className="mt-4 px-5 py-2 bg-white hover:bg-slate-50 text-slate-700 font-semibold rounded-full border border-slate-300 shadow-sm transition duration-200 text-sm flex items-center gap-2 cursor-pointer active:scale-95"
          >
            Remove Picture
          ...
          <div className="space-y-4">
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                ...
                <span className={`text-xs font-medium ${60 - (profile.about?.length || 0) <= 10 ? "text-red-500" : "text-slate-500"}`}>
                  {60 - (profile.about?.length || 0)} characters left
                </span>
              </div>
